...
class DenoteModel(nn.Module):
    def __init__(self, pars):
        ...
        self.cnn = nn.Conv1d(self.word_embed_size, self.word_out_channel, 3)
        self.cnn_norm = nn.BatchNorm1d(self.word_out_channel)

        # attention
        self.f_linear = nn.Linear(self.word_out_channel, self.word_out_channel)
        self.g_linear = nn.Linear(self.word_out_channel, self.word_out_channel)
        self.h_linear = nn.Linear(self.word_out_channel, self.word_out_channel)
    ...
    def forward(self, inps):
        ...
        padding = self.cnn.kernel_size[0] - 1
        cnn_ip = fun.pad(cnn_ip, (0, padding))
        cnn_out = self.cnn(cnn_ip)
        cnn_out = torch.transpose(cnn_out, 1, 2)
        cnn_out = fun.relu(cnn_out)

        out = self.batch_norm(self.cnn_norm, cnn_out)
