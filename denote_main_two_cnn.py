...
# @Time    : 18-6-19
# @Author  : wang shen
# @File    : denote_main_two_cnn.py

"""
cnn_2 + cnn_3 + attention
denote main is first step:
input is sentence,
output is denote and type
"""
import torch
from src.denote_model_hit import DenoteModel
...
def train_epoch(epoch, model, data_set, opt, batch_size, device):
    ...
    for samples in generate_batch_sample_iter(data_set, batch_size):
        ...
        p, r, f, _ = precision_recall_fscore_support(iter_kind, iter_kind_prd, average="macro")
        ...
        p, r, f, _ = precision_recall_fscore_support(iter_denote, iter_denote_prd, average="macro")
...
def eval_epoch(epoch, model, data_set, batch_size, device, log_path=None):
    global global_step, collect_kind, collect_denote_sklearn
    model.eval()
    e_loss, nb = 0.0, 0
    ...
    for samples in generate_batch_sample_iter(data_set, batch_size, False, False):
        ...
        iter_kind.extend(np.reshape([node["type_idx"] for node in samples], (-1)).tolist())
        iter_kind_prd.extend(prd_kind.tolist())
        p_k, r_k, f_k, _ = precision_recall_fscore_support(iter_kind, iter_kind_prd, average="macro")
        print("type---P: %.6f, R: %.6f, F: %.6f" % (p_k, r_k, f_k))
        print("type accuracy: %.6f" % accuracy_score(iter_kind, iter_kind_prd))

        iter_denote, iter_denote_prd = arrangement(iter_denote, iter_denote_prd, samples, prd_denote.tolist(), "denote")
        p_d_s, r_d_s, f_d_s, _ = precision_recall_fscore_support(iter_denote, iter_denote_prd, average="macro")
        print("denote---P: {:.6f}, R: {:.6f}, F: {:.6f}".format(p_d_s, r_d_s, f_d_s))
    ...
    if log_path:
        ...
        with open(log_path, "w") as fp:
            ...
            for d in collect_denote_sklearn:
                fp.write("%d\t%.6f\t%.6f\t%.6f\n" % (d[0], d[1], d[2], d[3]))
            fp.write("\n")
            fp.write("\n")
...
if __name__ == '__main__':
    global_step = 0
    collect_denote_sklearn, collect_kind = [], []
    main()
    print(collect_kind[0])
    print(collect_denote_sklearn[0])
